...
class ViewController: UIViewController {
    ...
    @IBOutlet weak var siStopButton: UIButton!
    @IBOutlet weak var guStopButton: UIButton!
    @IBOutlet weak var myTextView: UITextView!
    @IBAction func startTrasncribing(_ sender: Any) {
        transcribeButton.isEnabled = false
        siStopButton.isEnabled = true
        guStopButton.isEnabled = true
        try! startSession()
    }
    @IBAction func stopTranscribing(_ sender: Any) {
        if audioEngine.isRunning{
            ...
            siStopButton.isEnabled = false
            guStopButton.isEnabled = false
        }
        
        switch (self.myTextView.text) {
        case "서울":
            self.sidoCd.setTitle(self.myTextView.text, for: .normal)
    ...
    @IBAction func stopTranscribing2(_ sender: Any) {
        if audioEngine.isRunning{
            audioEngine.stop()
            speechRecognitionRequest?.endAudio()
            transcribeButton.isEnabled = true
    ...
    func startSession() throws{
        ...
        speechRecognitionTask = speechRecognizer.recognitionTask(with: recognitionRequest){result, error in var
    ...
    var urlEndDate : String = "&endde=20190617"
